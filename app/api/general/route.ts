...
const actionTools = [
  ...
  { name: "send_purse", description: "Komşu krallığa gizli bir kese gönderir. Üç hedef var: 'commons' → 600 altın halkının arasına dağıtılır ve orada örgütlü bir hizip büyütür; 'garrison' → 600 altın kışlasına sokulur ve asker huzursuzluğu enjekte eder; 'market' → 900 yiyecek (ya da eşdeğer mal) hedefin pazarına yığılır ve SATIŞ getirisini düşürür (hedef bu maldan ne ambar doldurabilir ne rıza kazanır). Fiyat sabittir, miktar seçilemez ve BAŞARI İHTİMALİ YOKTUR — etki kesindir. Hedef karşı-istihbarat nöbeti kurmuşsa kimliğimiz açığa çıkar, itibarımız düşer ve imzalı barışımız varsa anlaşma bozulur. Hedefi yalnızca neighbors listesindeki ordinal ile belirt. Tek kese ne orduyu dağıtır ne krallığı devirir; sabotaj aracıdır.", parameters: { type: "object", properties: { target_ordinal: { type: "integer", minimum: 1, maximum: 40 }, target: { type: "string", enum: ["commons", "garrison", "market"] }, resource: { type: "string", enum: ["food", "wood", "stone", "iron", "ale"], description: "Yalnızca 'market' hedefinde: yığılacak ve ambarımızdan çıkacak mal." }, confirmed_risk: { type: "boolean" } }, required: ["target_ordinal", "target"], additionalProperties: false } },
