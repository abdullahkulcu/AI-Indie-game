...
// Halkın meclise araya girmesinin eşiği, tekrar freni ve General'e gönderilen
// sohbet geçmişinin süzgeci SAF bir modülde yaşar ve testlidir
// (tests/populace-interjection.test.ts). Buradaki JSX test edilemez; o kural
// edilebilir olmalı, çünkü yanlış kurulduğunda General halkın şikâyetini
// kendi sözü sanır.
import { generalHistory, interjectionKey, interjectionSpeaker, pickInterjections, type ChatLine, type InterjectionDemand } from "./populace-interjection";
...
import { buildOptions, costFor, keep, materialScaleOf, rates, servedRations, tick } from "@/engine/tick";
...
// Zafer skorunun formülü, satır etiketleri ve doktrin okunuşu motorda yaşar
// (engine/victory.ts). Panel hiçbir katsayıyı elle tekrar yazmaz; skoru CANLI
// oyun durumu ile sunucudan gelen kese defterinden aynı fonksiyonla hesaplar.
import { victoryScore, type VictoryPurse } from "@/engine/victory";
// Kıyasın sözlüğü motordan gelir: hangi ölçütler, her ölçütte "iyi" hangi yön
// (yüksek vergi KÖTÜ) ve ortalamanın açıklanabilmesi için gereken en az sancak
// sayısı. Panel bunları elle tekrar yazmaz.
import { COMPARE_METRICS, COMPARE_MIN_SAMPLE, compareValuesOf, compareVerdict, type CompareMetric } from "@/engine/comparison";
...
import { MAX_HOURS, MAX_MESSAGE_LENGTH, MAX_TRIBUTE_AMOUNT, MAX_TRIBUTE_RATE_PERCENT, TRIBUTE_RESOURCES, canProposeTerms, canSpeak, carriesTribute, isClosedTable, isTimedOut, tributeRateFromPercent, type Negotiation, type NegotiationStatus, type NegotiationTopic, type Side, type Terms } from "@/engine/negotiation";
import type { Game, GameAction, TerrainId as EngineTerrainId } from "@/engine/types";


type Tab="meclis"|"binalar"|"halk"|"ordu"|"istihbarat"|"defter"|"diyar"; type Setup="welcome"|"channel"|"kingdom"|"general";
...
// Tipin TEK kaynağı artık `components/populace-interjection.ts`: balon eşiği o
// tipin alanlarını (severity, since) okuyor, panel kendi kopyasını tutmaz.
type PopulaceDemand=InterjectionDemand;
...
/** GET /api/world → `victory`. Skor burada DEĞİL motorda hesaplanır; sunucu
 *  yalnızca kendi kese defterimizi ve (gizlilik alt sınırı sağlanıyorsa)
 *  sıramızı gönderir — komşunun skoru ve kimliği inmez. */
type VictoryStanding={purses:VictoryPurse[];caught:number;rank:number|null;ranked:number};
// Channel kıyası: SUNUCUDAN yalnızca ortalama iner (server/world-projection.ts →
// channelAverages). Kimin hangi değere sahip olduğu hiç gelmez; aday sayısı
// gizlilik alt sınırının altındaysa `averages` bile null gelir.
type ChannelCompare={counted:number;protectedOut:number;averages:Record<CompareMetric,number>|null};
// Channel pazar endeksi (plan belgesi Fikir 24): mal başına tek bir fiyat
// çarpanı. SUNUCUDAN iner (app/api/world → channelPriceIndex) ve kaydın içinde
// DURMAZ — istemcinin bildirebileceği bir alan olsaydı Kral endeksi kendi
// lehine yazıp fiyatı kırardı (bkz. server/save-validation.ts, "HALKIN DEFTERİ
// SUNUCUNUN"). Gizlilik alt sınırının altında null gelir; o zaman endeks nötrdür.
type ChannelIndex=Partial<Record<TradeKey,number>>;
// Kıyas satırlarının Türkçe başlıkları. Ölçüt listesi ve yön motorda; burada
// yalnızca sözcükler var. `Record` tipi yeni bir ölçüt eklendiğinde bu haritayı
// derleme hatasıyla zorlar, satır sessizce eksik kalmaz.
const compareLabels:Record<CompareMetric,{name:string;percent:boolean}>={popularity:{name:"Rıza",percent:false},foodRation:{name:"Yiyecek istihkakı",percent:true},taxRate:{name:"Vergi oranı",percent:true},factionPressure:{name:"Muhalefet baskısı",percent:false}};
const compareText=(metric:CompareMetric,value:number)=>compareLabels[metric].percent?`%${value}`:`${value}`;
...
// `mood`: DERİN GÖZETLEME raporunun tek ek alanı (plan belgesi Fikir 5) —
// SAYI DEĞİL kaba durum etiketi ("Huzursuz"). Standart keşifte null/undefined
// gelir; eski raporlar bu alanı hiç taşımaz.
type IntelReport={name:string;ruler:string;terrain:string;keepLevel:number;population:number;buildingCount:number;army:number;mood?:string|null};
type WorldKingdom={id:string;name:string|null;terrain:string;position:{x:number;z:number};ring:number;discovered:boolean;mission:{status:"pending"|"succeeded"|"failed"|"detected";completesAt:number;successChance:number;kind?:"scout"|"deep"}|null;report:IntelReport|null;reportAt?:number|null;reportStale?:boolean};
...
/**
 * Masanın tek satırlık durumu. CANLI masa da GEÇMİŞ masa da buradan okur:
 * etiket zinciri iki yerde elle yazılsaydı geçmiş listesi bir gün canlı
 * listeden farklı bir şey derdi. (Etiketlerin tek yerde durduğunu
 * tests/negotiation-expiry.test.ts iddia ediyor; bu yorumda etiket METNİ
 * geçmiyor, yoksa sayım ikiye çıkardı.)
 */
function masaDurumu(table:NegotiationTable,at:number,maxTurns:number){
 if(table.status==="awaiting_king")return "ŞART SUNULDU";
 if(table.status==="agreed")return "ANLAŞILDI";
 if(table.status==="declined")return "REDDEDİLDİ";
 // isTimedOut zaten "expired" durumunu da doğru sayar (bkz. engine/negotiation.ts).
 if(isTimedOut({status:table.status as NegotiationStatus,expiresAt:table.expiresAt},at))return "SÜRESİ DOLDU";
 return `${table.turns}/${maxTurns} söz`;
}
...
// `influence`: ORTAK MADENDE BÖLGE SAHİPLİĞİ (plan belgesi Fikir 22).
// SUNUCUDAN iner ve salt okunurdur; pay oranı da pencere de motorda karara
// bağlanır (engine/mine.ts). Sahibin ortalaması (`workerAvg`) İNMEZ: o sayı
// "kaç işçiyle geçebilirim" hesabını birebir çözerdi.
// `find`: DAMAR TÜKENİNCE ÇIKAN FIRSAT (plan belgesi Fikir 23). Tür, miktar ve
// konum SUNUCUDA tohumdan türetilir (engine/mine.ts → mineFindOf); panel
// yalnızca gösterir, hiçbir sayı göndermez.
type MineFindView={kind:string;label:string;text:string;resource:string;amount:number;offset:{x:number;z:number};status:string;claimedBy:string|null;claimedName:string|null;self:boolean;canClaim:boolean;appearedAt:number};
type MineInfluenceView={userId:string;name:string;self:boolean;cut:number;windowGameHours:number;windowEndsAt:number};
type SharedMine={personalCap?:number;channelSlots?:number;orePerWorkerHour?:number;mine:{id:string;name:string;oreRemaining:number;extractedOre:number;totalWorkers:number;position:{x:number;z:number}};participants:Array<{id:string;name:string;workers:number;deliveredOre?:number;self:boolean}>;influence?:MineInfluenceView|null;find?:MineFindView|null};
...
export default function KingdomGame(){
 ...
 // kendisi yapar. Sayfa yenilenince sıfırlanır — sohbetin kendisi de kalıcı
 // olmadığı için (`chat` yalnızca istemci-içi) bu tutarlıdır: yeni bir sohbette
 // halk süregelen acil talebi için bir kez yeniden söz alır.
 const spokenInterjections=useRef<Set<string>>(new Set());
 ...
 // MODELİN TEK KAYNAĞI sunucudaki saklı kimlik bilgisidir: General'in isteği
 // onunla yapılır ve faturayı o belirler. Kayıttaki kopya (game.model) yalnızca
 // gösterim içindir ve başka bir sekmeden/cihazdan model değiştirildiğinde
 // bayatlıyordu; panel bir modeli gösterirken sunucu başkasını kullanıyordu.
 ...
 useEffect(()=>{if(!game?.channelId){setNegotiationTables([]);setEnvoyHistory([]);setAgreementList([]);return}
  let active=true;
  const load=async()=>{try{const response=await fetch("/api/negotiate",{cache:"no-store"});
    if(!response.ok)return;const data=await response.json() as {tables?:NegotiationTable[];history?:NegotiationTable[];agreements?:Agreement[];limits?:{maxTurns:number;maxOpen:number};acceptsNegotiation?:boolean};
    if(!active)return;
    ...
    const bekleyenMasa=tables.filter(entry=>entry.canAccept
      ||(!isClosedTable(entry.status as NegotiationStatus)&&!isTimedOut({status:entry.status as NegotiationStatus,expiresAt:entry.expiresAt},Date.now())&&entry.messages.length>0&&!entry.messages[entry.messages.length-1].mine));
    ...
    setNegotiationTables(tables);setEnvoyHistory(data.history??[]);setAgreementList(data.agreements??[])}catch{/* sessiz: masa yoksa panel de yok */}};
 ...
 function found(connected:boolean,introduction?:string){const t=Date.now();const g:Game=foundKingdom({kingdomName:name.trim(),rulerName:ruler.trim(),channel:selected.name,channelId:selected.id,speed:selected.speed,terrain,provider:connected?provider:null,model:connected?model:null,generalConnected:connected},t);// Katılım beklenmeden gönderiliyordu: başarısız olduğunda krallık kuruluyor
    ...
    void(async()=>{try{const response=await fetch("/api/channels",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({channelId:selected.id,kingdomName:name.trim()})});
      ...
      else setServerChannelId(selected.id)}catch{setWorldError("Channel katılımı kaydedilemedi; komşular ve ortak saha görünmeyecek.")}})();setGame(g);setChat([{who:connected?generalName:"Saray Kâtibi",text:connected?(introduction||"Bağlantı doğrulandı. İlk hedefinizi ve yönetim doktrinimizi belirleyin."):"General henüz sessiz. Üretim ve kuyruklar çalışır; yeni yönetim kararları için BYOK General'i bağlamalısınız.",kind:connected?"general":"clerk"}]);if(!localStorage.getItem("demirkale.tutorial.done"))setTutorialStep(0)}
 function kingdomContext(g:Game|null){if(!g){
  // Krallık henüz kurulmadan General'e gönderilen yedek bağlam. Kaynaklar ve
  // saatlik oranlar burada ELLE yazılıydı (hourlyRates:{gold:3.3,food:14.5,…});
  // motorun formülü değişince sessizce sapıyordu. Artık kanonik kuruluş
  // durumundan türetilir ve oranlar motorun `rates()` fonksiyonundan okunur.
  ...
  const pazar=marketState(g,Date.now(),channelIndex??undefined);
 ...
 // "deep_scout" = DERİN GÖZETLEME (Fikir 5): standart keşfin bir çeşidi değil,
 // ayrı ve bedelli bir görev. Bedeli ve ihtimalleri SUNUCU belirler; buradan
 // hiçbir sayı gönderilmez, gelen cevaptaki sayı yalnızca gösterilir.
 async function worldAction(action:"scout"|"deep_scout"|"defend",targetId?:string){if(!game?.channelId||worldBusy)return;setWorldBusy(true);try{const response=await fetch("/api/world",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,channelId:game.channelId,targetId})});const data=await response.json() as {error?:string;completesAt?:number;successChance?:number;activeUntil?:number;kind?:"scout"|"deep";cost?:number};if(!response.ok)throw new Error(data.error||"Dünya emri uygulanamadı.");if(action==="defend")setWorldDefense({active:true,activeUntil:data.activeUntil??null});else setOtherKingdoms(items=>items.map(item=>item.id===targetId?{...item,mission:{status:"pending",completesAt:data.completesAt??Date.now(),successChance:data.successChance??10,kind:data.kind??"scout"}}:item));setToast(action==="defend"?"Karşı-istihbarat nöbeti bir saatliğine kuruldu.":action==="deep_scout"?`Derin gözetleme ajanı yola çıktı · ${data.cost??deepIntel.deepCost} altın · başarı ihtimali %${data.successChance??0}.`:`Ajan yola çıktı · başarı ihtimali %${data.successChance??10}.`)}catch(error){setToast(error instanceof Error?error.message:"Dünya emri uygulanamadı.")}finally{setWorldBusy(false)}}
...
async function openNegotiation(){
 ...
 async function mineAction(action:"join"|"leave"|"claim_find"){if(!game?.channelId||worldBusy)return;const minersToSend=Math.max(1,Math.min(sharedMine?.personalCap??3,Math.round(game.population*.1)));setWorldBusy(true);try{const response=await fetch("/api/mine",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,channelId:game.channelId,workers:minersToSend})});const data=await response.json() as {error?:string;workers?:number;claimed?:boolean;label?:string;amount?:number;resource?:string};if(!response.ok)throw new Error(data.error||"Maden emri uygulanamadı.");
   // Fırsatı almak işçi sayısını değiştirmez; yalnızca ödül sunucuda kayda
   // yazılır ve bir sonraki 409'da istemciye iner (haraç/cevher deseni).
   if(action==="claim_find"){setToast(`${data.label??"Fırsat"} sizin oldu · ${data.amount??0} ${data.resource??""} ambara indirildi.`);}
   else{setGame(current=>current?{...current,mineWorkers:data.workers??0}:current);setToast(action==="join"?`Ortak madene ${data.workers??minersToSend} işçi gönderildi; saatte ~${Math.round((data.workers??minersToSend)*(sharedMine?.orePerWorkerHour??0))} cevher bekleniyor, o eller tarladan eksildi.`:"İşçiler ortak madenden çekildi, kalan cevher ambara indirildi.");}const mineResponse=await fetch(`/api/mine?channelId=${encodeURIComponent(game.channelId)}`,{cache:"no-store"});if(mineResponse.ok)setSharedMine(await mineResponse.json() as SharedMine)}catch(error){setToast(error instanceof Error?error.message:"Maden emri uygulanamadı.")}finally{setWorldBusy(false)}}
 // Sunucu FİİLEN hangi modeli kullandıysa onu geri bildirir; panel kendi
 ...
 // altında YENİDEN ŞİFRELER; sütunu tek başına güncellemek çözmeyi bozar ve
 // General susardı.
 async function patchByokModel(){const response=await fetch("/api/byok",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({provider,model})});const data=await response.json() as {error?:string;model?:string};if(!response.ok)throw new Error(data.error||"Model değiştirilemedi.");setStoredModel(data.model??model)}
 ...
 async function reconnectGeneral(){if(!game||(!apiKey.trim()&&!storedByok))return setGeneralError("API anahtarını girmelisiniz.");
  // Sağlayıcı değişimi yeni anahtar ister: OpenAI anahtarı Anthropic'te çalışmaz.
  if(!apiKey.trim()&&storedByok&&storedProvider&&provider!==storedProvider)return setGeneralError("Sağlayıcıyı değiştirmek için API anahtarını yeniden girmelisiniz.");
  setConnecting(true);setGeneralError("");
  try{
   // Model değişikliği ÖNCE sunucuya yazılır; yoksa test çağrısı eski modele
   // gider ve panel yeni modeli göstererek ayrışırdı.
   if(!apiKey.trim()&&storedByok&&model!==storedModel)await patchByokModel();
   const intro=await askGeneral("test");
   if(apiKey.trim())await persistByok();
   setGame({...game,provider:intro.used.provider,model:intro.used.model,generalConnected:true});
   setChat(x=>[...x,{who:generalName,text:intro.text,kind:"general"}]);setShowConnect(false);setToast("General bağlantısı hesaba şifreli olarak kaydedildi.")
  }catch(error){setGeneralError(error instanceof Error?error.message:"Bağlantı kurulamadı.")}finally{setConnecting(false)}}
 ...
 async function submitOrder(order:string){if(!game||!order.trim()||connecting)return;setChat(x=>[...x,{who:game.rulerName,text:order.trim(),kind:"king"}]);setMessage("");setTab("meclis");if(!game.generalConnected){setChat(x=>[...x,{who:"Saray Kâtibi",text:"General bağlantısı etkin değil. Hesabındaki BYOK bağlantısını yenilemelisin.",kind:"clerk"}]);setShowConnect(true);return}setConnecting(true);try{const answer=await askGeneral("chat",order.trim()),execution=applyActions(game,answer.actions,Date.now(),channelIndex??undefined);if(answer.actions.length)setGame(execution.game);const remoteResults=await executeRemoteActions(execution.remote,game.channelId??availableChannels.find(channel=>channel.name===game.channel)?.id);const merged=[...execution.results,...remoteResults];const outcome=merged.length?`\n\n${merged.join("\n")}`:"";
  // HALK ARAYA GİRER (plan belgesi Fikir 16). Balon General'in cevabının
  // ÖNÜNE konur: halkın gücü sözü kesmesindedir, cevabı beklemesinde değil.
  // Eşik ve tekrar freni `pickInterjections`'ta; buradaki iş yalnızca
  // konuşanları deftere yazıp balonları sohbete eklemek.
  const interjections=pickInterjections(answer.populaceDemands,spokenInterjections.current);
  for(const demand of interjections)spokenInterjections.current.add(interjectionKey(demand));
  setChat(x=>[...x,...interjections.map(demand=>({who:interjectionSpeaker(demand.voice),text:demand.text,kind:"populace" as const,voice:demand.voice})),{who:generalName,text:`${answer.text}${outcome}`,kind:"general" as const}]);if(remoteResults.some(line=>line.startsWith("✓")))worldRefresh.current?.()}catch(error){setChat(x=>[...x,{who:"Saray Kâtibi",text:error instanceof Error?error.message:"General yanıt veremedi.",kind:"clerk"}]);setGame({...game,generalConnected:false});setShowConnect(true)}finally{setConnecting(false)}}
 // Kral politikayı doğrudan çevirir; General uygulamaz ama görüşünü söyler.
 function setPolicy(key:PolicyKey,value:number){
  if(!game)return;
  const result=applyPolicy(game,{key,value:clampPolicy(key,value)},Date.now());
  setGame(result.game);
  setChat(x=>[...x,{who:generalName,text:result.comment,kind:"general"}]);
  setTab("meclis");
 }
 ...
 <section className="world-stage"><KingdomScene night={night} keepLevel={lv} developed={lv>=3&&game.buildings.length>=8} terrain={game.terrain} constructionName={game.queue?.kind==="building"?game.queue.name:undefined} buildings={game.buildings} population={game.population} capacity={game.capacity} kingdoms={nearbyKingdoms} sharedMine={nearbyMine} army={armySize(game.units)} watchRatio={watchRatioOf(game)} mood={mood.id} autoRotate={autoRotate} seedKey={`${game.kingdomName}|${game.foundedAt}`}/><div className="scene-title"><button className="world-view-toggle" onClick={()=>setWorldView(true)}>CHANNEL HARİTASINI AÇ</button><button className={autoRotate?"rotate-toggle spinning":"rotate-toggle"} title={autoRotate?"Kamera dönüşünü durdur":"Kamerayı yeniden döndür"} onClick={()=>setAutoRotate(x=>!x)}>{autoRotate?"⟳ DÖNÜŞ AÇIK":"⏸ DÖNÜŞ DURDU"}</button><span>YENİ BAŞKENT · {(terrainCatalog[game.terrain]??terrainCatalog.plain).label}</span><h2>{game.kingdomName}</h2><p>Kale Sv.{lv} · {game.buildings.length} yapı · {Object.values(game.units).reduce((a,b)=>a+b,0)} asker</p><small>{(terrainCatalog[game.terrain]??terrainCatalog.plain).bonus} · {otherKingdoms.length} komşu sancak genel haritada</small></div>{Boolean(game.channelId)&&(()=>{
  ...
  return <div className={negotiationOpen?"envoy-dock open":"envoy-dock"}>
   ...
   {negotiationOpen&&<div className="envoy-sheet">
    <div className="envoy-head"><b>Elçilik</b><small>{negotiationTables.length} masa · {agreementList.length} anlaşma{envoyHistory.length>0&&` · ${envoyHistory.length} geçmiş`}</small><button onClick={()=>setNegotiationOpen(false)}>✕</button></div>
    ...
    {negotiationTables.map((table,index)=>{
     ...
     return <article className={table.canAccept?"envoy-table pending":"envoy-table"} key={table.id}>
      <div className="envoy-title"><b>#{index+1} {table.counterpart}</b><small>{masaDurumu(table,now,envoyLimits.maxTurns)}</small></div>
   ...
   {/* GEÇMİŞ MASALAR. Kapanmış masa artık önündeki listede durmuyor: Kral onu
       "cevap bekliyor" sanıp arıyordu. Burada kapanış sırasıyla, katlanmış
       hâlde duruyor; açan tutanağı okuyabilir. Sayı gösterilmesi şart, yoksa
       geçmişin var olduğu hiç belli olmaz. */}
   {envoyHistory.length>0&&<div className="envoy-history">
    <button className="envoy-history-tab" onClick={()=>setHistoryOpen(x=>!x)} aria-expanded={historyOpen}>
     <b>GEÇMİŞ MASALAR</b><i>{envoyHistory.length}</i><span>{historyOpen?"▾":"▸"}</span>
    </button>
    {historyOpen&&<div className="envoy-history-list">
     {envoyHistory.map(table=><details className="envoy-past" key={table.id}>
      <summary><b>{table.counterpart}</b><em>{masaDurumu(table,now,envoyLimits.maxTurns)}</em></summary>
      {table.proposed&&<div className="envoy-terms"><span>SON ŞART</span><b>{özet(table.proposed)}</b></div>}
      {table.messages.length>0
       ?<div className="envoy-log">{table.messages.map((message,i)=><p className={message.mine?"mine":""} key={i}>{message.body}</p>)}</div>
       :<p className="envoy-past-empty">Bu masada hiç söz söylenmedi.</p>}
     </details>)}
    </div>}
   </div>}
   </div>}
  </div>})()}
{(()=>{const pazar=marketState(game,now,channelIndex??undefined);
   ...
   {marketOpen&&<div className="market-sheet">
    {!pazar.level
     ...
     :<><div className="market-head"><b>Pazar Sv.{pazar.level}</b><small>{pazar.freeSlots}/{pazar.slots} yuva boş · {pazar.left} birim hacim</small><button onClick={()=>setMarketOpen(false)}>✕</button></div>
       ...
       {/* CHANNEL PAZAR ENDEKSİ (Fikir 24). Fiyatı gerçekten oynatan bir etki
           gösterilmeden bırakılamaz: Kral neden farklı bir fiyat gördüğünü
           anlamazsa oyunu değil hatayı okur. Yığının tersine bu etki İKİ KOLA
           da işler ve halkın geçim yüküne hiç dokunmaz. */}
       {pazar.indexed&&<p className="market-index">Komşu sancakların tezgâhı buraya da vuruyor: {rows.filter(([key])=>Math.abs((pazar.index[key]??1)-1)>1e-9).map(([key,label])=>`${label} ${(pazar.index[key]??1)>1?"+":"−"}%${Math.abs(((pazar.index[key]??1)-1)*100).toFixed(1)}`).join(", ")}. Channel çapında bir mal bollaştıkça burada da ucuzluyor, kıtlaştıkça pahalanıyor; etki hem alışa hem satışa işliyor ve halkın geçim yükünü değiştirmiyor.</p>}
...
{game.queue&&<div className="active-queue"><span>{game.queue.kind==="building"?"⚒ HARİTADA İNŞAAT":"EĞİTİM DEVAM EDİYOR"}</span><b>{game.queue.name}</b><strong>{left(game.queue.completesAt,now)}</strong><div className="queue-progress"><i style={{width:`${game.queue.startedAt?Math.max(4,Math.min(100,(now-game.queue.startedAt)/(game.queue.completesAt-game.queue.startedAt)*100)):8}%`}}/></div></div>}
 <aside className="council-panel"><div className="general-bars">{[["Sadakat",game.loyalty],["Halkın Rızası",Math.round(game.popularity)],["İtibar",game.reputation]].map(([label,value])=><div className="stat" key={label}><span>{label}</span><div><i style={{width:`${value}%`}}/></div><b>{value}</b></div>)}</div><nav className="tabs seven">{(["meclis","binalar","halk","ordu","istihbarat","defter","diyar"] as Tab[]).map(x=><button key={x} className={tab===x?`${x} active`:x} onClick={()=>setTab(x)}>{x.toLocaleUpperCase("tr")}</button>)}</nav><div className="panel-content">
 ...
 {/* Balonun kimliği artık adından DEĞİL `kind` alanından okunur; halkın sesi
     General'in balonundan görsel olarak da ayrıdır. Halkın cümlesi
     `RichMessage`'tan geçmez, ham metin olarak basılır: o metin Halk-AI ürünü
     ve markdown biçimlendirmesiyle (kalın başlık, tablo) Kral'ın gözünde
     kendine General'in yetkisine benzer bir ağırlık kurmasına gerek yok —
     halk tek cümleyle konuşur. */}
 <div className="chat-log">{chat.map((x,i)=>x.kind==="populace"
   ?<div className={`message populace ${x.voice??"commons"}`} key={i}><b>{x.who}</b><em>ACİL</em><p>{x.text}</p><small>{"General'in ağzından değil, doğrudan Kral'a. Karşılıksız kalırsa muhalefete akar."}</small></div>
   :<div className={x.kind==="king"?"message king":"message"} key={i}><b>{x.who}</b><RichMessage text={x.text}/></div>)}{connecting&&<div className="message thinking"><b>GENERAL ALDRIC</b><p>Haritayı ve defterleri inceliyor…</p></div>}<div ref={chatEnd}/></div><form className="chat-form" onSubmit={send}><input value={message} onChange={e=>setMessage(e.target.value)} placeholder="Generalinize buyruğunuzu iletin…" disabled={connecting}/><button disabled={connecting}>✦</button></form></div>}
 {tab==="binalar"&&(()=>{const secenekler=buildOptions(game),carpan=materialScaleOf(game.speed);
   return <div className="list-content"><div className="section-head"><span>{game.buildings.length} / {6+(lv-1)*2} YAPI{carpan>1?` · MALZEME ×${carpan}`:""}</span><b>{game.queue?.kind==="building"?"1 kuyrukta":"General planlayabilir"}</b></div><div className="general-only-note"><b>Binaları General yönetir</b><span>Bir hedef ver veya onu ikna et: “Önce yiyeceği güvenceye al, sonra Meydan kur.”</span></div><button className="keep-card" disabled={lv>=6||connecting} onClick={()=>void submitOrder(`Kale yapısını Sv.${lv+1} seviyesine yükselt.`)}><span>♜</span><div><b>Kale Sv.{lv}{lv<6?` · sıradaki ${lv+1}`:" · Tavan"}</b><small>İdari kapasite ve emir kotası</small></div><strong>{lv>=6?"TAVAN":"EMİR VER"}</strong></button>{catalog.map(item=>{const current=game.buildings.find(b=>b.type===item.type)?.level??0,locked=item.unlock>lv,maxed=current>=MAX_BUILDING_LEVEL,/* Kilitli yapı `buildOptions`tan DÖNMEZ (o liste yalnızca şimdi emredilebilenleri verir), bu yüzden cost {} kalıyordu ve "maliyeti ·" arkası boş basılıyordu. Oyuncu Pazar'a ne biriktireceğini öğrenemiyordu. Yedek hesap `buildOptions`ın kullandığı AYNI `costFor` ile yapılır — formül ikinci bir yere yazılmaz. */cost=secenekler.find(option=>option.type===item.type)?.cost??costFor(item.cost,current,carpan);return <button className={`build-action ${locked?"locked":""}`} disabled={locked||maxed||connecting} onClick={()=>void submitOrder(current?`${item.name} yapısını Sv.${current+1} seviyesine yükselt.`:`${item.name} yapısını kur.`)} key={item.type}><span>{item.category}</span><div><b>{item.name} {current?`Sv.${current}`:"Kurulmadı"}</b><small>{locked?`Kale Sv.${item.unlock} gerekli`:item.detail}</small><em>{maxed?`Sv.${MAX_BUILDING_LEVEL} tavanı · daha ileri yükseltilemez`:`Sonraki emir maliyeti · ${Object.entries(cost).map(([k,v])=>`${(resourceLabels.find(([id])=>id===k)?.[1]??k).toLocaleLowerCase("tr-TR")} ${v}`).join(" · ")}`}</em></div><strong>{maxed?"TAVAN":locked?"KİLİTLİ":"EMİR VER"}</strong></button>})}</div>})()}
 ...
 {tab==="ordu"&&<div className="army-panel"><div className="section-head"><span>GARNİZON · RAPOR</span><b>{game.units.spearman??0} asker</b></div><div className="general-only-note"><b>Orduyu General yönetir</b><span>Kaç asker istediğini veya savunma hedefini söyle. General nüfus ve yiyecek riski varsa itiraz eder.</span></div><div className="empty-army"><span>⚔</span><h3>{game.buildings.some(b=>b.type==="barracks")?"Kışla Generalin emrini bekliyor":"Henüz Kışlanız yok"}</h3><p>{game.buildings.some(b=>b.type==="barracks")?"Mızrakçı başına: 8 altın · 10 yiyecek · 1 demir":"Kışla kurulması için General'e stratejik gerekçeni ilet."}</p></div><div className="unit-row"><span>Mızrakçı</span><b>{game.units.spearman??0}</b><small>Savunma 16 · Hız 6</small></div>
  {(()=>{const d=defenseOf(game),watch=watchRatioOf(game),raids=game.notices.filter(n=>n.kind==="AKIN").slice(0,3);
   ...
   <div className="raid-log">
    ...
    {/* Haydut yönlendirmenin okunduğu yer artık İSTİHBARAT sekmesi: "eşkıya
        kimin eliyle bu tarafa çekiliyor" bir garnizon raporu değil, komşuya
        dair bir istihbarat bulgusudur. Buraya kopyası BIRAKILMADI. */}
    {raids.length?raids.map((n,i)=><p key={i}>{n.text}</p>):<p className="quiet">Dağlardan henüz akın gelmedi.</p>}
   </div></>;})()}
  </div>}
 {/* İSTİHBARAT — komşuya YÖNELİK araçların tek yeri (plan belgesi, Fikir 3
     ve altındaki "Karar (2026-08-22)"). Karşı-istihbarat, dış kese ve haydut
     yönlendirme buraya "diyar" ve "ordu" sekmelerinden TAŞINDI; eski
     yerlerinde kopyaları bırakılmadı.

     MUHALEFET PANELİ BİLİNÇLİ OLARAK "halk" SEKMESİNDE KALDI: muhalefet KENDİ
     halkımızın rızasıyla ilgilidir ve tek çıkışı istihkak/vergi/konut
     kararlarıdır — onu bu sekmeye almak, halkı komşu gibi "hedef" sayan
     yanlış bir okuma üretirdi. Bu sekme dışa, "halk" sekmesi içe bakar. */}
 {tab==="istihbarat"&&(()=>{
  // Skor motorda hesaplanır: katsayılar, satır etiketleri ve doktrin okunuşu
  // engine/victory.ts'te TEK kaynakta. Girdi olarak CANLI oyun durumu ile
  // sunucunun gönderdiği kese defteri verilir.
  const score=victoryScore({purses:victory?.purses??[],caughtPurses:victory?.caught??0,game});
  const lure=lureAt(game,now);
  return <div className="realm">
   <div className="section-head"><span>İSTİHBARAT · ASKERSİZ SAVAŞ</span><b>{score.total} PUAN</b></div>
   <div className="world-intel">
    <div className="victory-score">
     <div className="section-head"><span>ZAFER SKORU · SEZON ÖLÇÜTÜ</span><b>{score.doctrine.label}</b></div>
     <div className="victory-columns">
      <div className="quiet"><span>ASKERSİZ ÜSTÜNLÜK</span><b>{score.quiet}</b></div>
      <div className="martial"><span>ASKERİ KATKI</span><b>{score.martial}</b></div>
      <div className="total"><span>TOPLAM</span><b>{score.total}</b></div>
     </div>
     <p className="victory-doctrine">{score.doctrine.note}</p>
     <div className="victory-lines">{score.lines.map(line=><article key={line.id} className={line.column}>
       <div><b>{line.label}</b><small>{line.detail}</small></div>
       <strong className={line.points>0?"gain":line.points<0?"loss":""}>{line.points>0?"+":""}{line.points}</strong>
      </article>)}</div>
     {/* Sıra sunucudan gelir ve gizlilik alt sınırına tabidir; komşunun skoru,
         adı ve kalemleri hiçbir koşulda inmez. */}
     <small className="victory-rank">{victory?.rank
       ? `Channel sıralaması: ${victory.rank} / ${victory.ranked} sancak. Komşuların skoru ve kimliği size inmez.`
       : `Sıralama verilmedi: sıra en az ${COMPARE_MIN_SAMPLE+1} sancakla açıklanır; kuruluş koruması süren sancaklar (ve koruması süren Kral) sayılmaz.`}</small>
     <small className="victory-note">Skor bir GÖSTERGEDİR: sezonu bitirmez, kimseyi kazanan ilan etmez. Sezon channel süresine bağlı kapanır.</small>
    </div>
    <div className="intel-defense"><div><b>KARŞI-İSTİHBARAT</b><span>{worldDefense.active?`Nöbet etkin · ${left(worldDefense.activeUntil??now,now)}`:"Ajan savunması kapalı"}</span>{incomingAlerts>0&&<em>{incomingAlerts} düşman ajanı tespit edildi</em>}</div><button disabled={worldBusy||worldDefense.active} onClick={()=>void worldAction("defend")}>{worldDefense.active?"NÖBETTE":"NÖBET KUR"}</button></div>
    ...
    <div className="intel-defense"><div><b>HAYDUT YÖNLENDİRME</b><span>{lure>0
      ?"Dağ yollarında tuhaf bir hareket var: akınlar olması gerekenden sık geliyor ve eşkıya hep bu tarafa çekiliyor. Akınların şiddeti değişmedi; sıklığı arttı. Kimin çektiği belli değil."
      :"Dağ yolları olağan; eşkıyayı bu tarafa çeken bir el görünmüyor."}</span><em>{lure>0
      ?"Sıklık arttığı için nöbet oranını Ordu sekmesinden gözden geçirin; yönlendirme kendiliğinden söner."
 ...
 {/* CHANNEL KIYASI — anonim. Sunucu yalnızca ortalamayı gönderir (kimin hangi
     ...
     sayı hiç üretilmez; o zaman burada da sayı yerine gerekçe görünür. */}
 {/* Kıyas hiç GELMEDİYSE (yerel oyun, channel bilgisi alınamadı) panel hiç
     çizilmez: "yeterli sancak yok" demek yanlış bir iddia olurdu. */}
 {channelCompare&&<div className="channel-compare"><div className="compare-head"><span>CHANNEL KIYASI · ANONİM</span><b>{channelCompare.averages?`${channelCompare.counted} SANCAK`:"YETERSİZ"}</b></div>{(()=>{const averages=channelCompare.averages;
   if(!averages)return <small className="compare-note">Kıyas için yeterli sancak yok: ortalama en az {COMPARE_MIN_SAMPLE} sancaktan hesaplanır. Daha azında &quot;ortalama&quot; tek bir komşunun defterini olduğu gibi ele verirdi.{channelCompare.protectedOut>0?` Kuruluş koruması süren ${channelCompare.protectedOut} sancak da sayılmadı.`:""}</small>;
   // Bizdeki değerler ortalamayla AYNI motor fonksiyonundan okunur; kıyas ise
   // gösterilen (yuvarlanmış) sayılar üzerinden yapılır, yoksa ekranda eşit
   // görünen iki sayının biri yeşil biri kırmızı olurdu.
   const mine=compareValuesOf(game);
   return <><div className="compare-grid"><i>ÖLÇÜT</i><i>BİZDE</i><i>ORTALAMA</i>{COMPARE_METRICS.map(metric=>{const ours=Math.round(mine[metric]),theirs=Math.round(averages[metric]);
     return <Fragment key={metric}><span>{compareLabels[metric].name}</span><b className={compareVerdict(metric,ours,theirs)}>{compareText(metric,ours)}</b><em>{compareText(metric,theirs)}</em></Fragment>})}</div>
     <small className="compare-note">Ortalamaya {channelCompare.counted} sancak girdi{channelCompare.protectedOut>0?`; kuruluş koruması süren ${channelCompare.protectedOut} sancak sayılmadı`:""}. Kendi krallığınız ortalamaya girmez ve hiçbir sancağın kendi değeri gösterilmez.</small></>})()}</div>}
 {sharedMine&&<div className="shared-mine"><span>⛏ CHANNEL ORTAK SAHASI</span><b>{sharedMine.mine.name}</b><small>{fmt(sharedMine.mine.extractedOre)} cevher çıkarıldı · {fmt(sharedMine.mine.oreRemaining)} kaldı · {sharedMine.mine.totalWorkers}/{sharedMine.channelSlots??60} yuva dolu</small><small className="mine-limit">Madenciler halkın içinden çıkar: nüfusunuzun en fazla %20&apos;si ({sharedMine.personalCap??0} kişi). Madendeki her el tarlada eksiktir.</small>{(()=>{const self=sharedMine.participants.find(worker=>worker.self),rate=sharedMine.orePerWorkerHour??0;
     return <small className="mine-limit">İşçi başına saatte {rate} cevher · {self?`${self.workers} işçiniz saatte ${self.workers*rate} demir getiriyor`:"şu an madende işçiniz yok"} · size teslim edilen toplam {fmt(self?.deliveredOre??0)} cevher. Cevher ambara sunucu tarafından yazılır, birikip toplu iner.</small>})()}{sharedMine.influence?<small className={sharedMine.influence.self?"mine-influence mine":"mine-influence"}>Bölge sahibi · <b>{sharedMine.influence.self?"sizin sancağınız":sharedMine.influence.name}</b> · ötekilerin ürettiğinin %{Math.round(sharedMine.influence.cut*100)}&apos;ini alır · sahiplik {sharedMine.influence.windowGameHours} oyun saatinde bir tartılır</small>:<small className="mine-influence">Bölge sahibi yok · damarda açık üstünlüğü olan bir sancak bulunmuyor</small>}{sharedMine.find?<div className={sharedMine.find.status==="claimed"?"mine-find claimed":"mine-find"}><span>DAMAR TÜKENDİ · YENİ FIRSAT</span><b>{sharedMine.find.label}</b><small>{sharedMine.find.text}</small><small>Madenin {sharedMine.find.offset.x>0?"doğusunda":"batısında"} ({sharedMine.find.offset.x}, {sharedMine.find.offset.z}) · {sharedMine.find.amount} {sharedMine.find.resource} · TEK KAZANAN alır</small>{sharedMine.find.status==="claimed"?<small className="mine-find-owner">{sharedMine.find.self?"Fırsatı siz aldınız.":`${sharedMine.find.claimedName??"Bilinmeyen bir sancak"} sizden önce ulaştı.`}</small>:<button disabled={worldBusy||!sharedMine.find.canClaim} onClick={()=>void mineAction("claim_find")}>{sharedMine.find.canClaim?"FIRSATA ULAŞ":"ULAŞMAK İÇİN MADENDE İŞÇİ GEREKİR"}</button>}</div>:null}<div>{sharedMine.participants.map(worker=><i key={worker.id} className={worker.id===sharedMine.influence?.userId?"owner":undefined}>{worker.name} · {worker.workers}{worker.deliveredOre?` · ${fmt(worker.deliveredOre)} cevher`:""}</i>)}</div><button disabled={worldBusy} onClick={()=>void mineAction(sharedMine.participants.some(worker=>worker.self)?"leave":"join")}>{sharedMine.participants.some(worker=>worker.self)?"İŞÇİLERİ ÇEK":`${Math.max(1,Math.min(sharedMine.personalCap??3,Math.round(game.population*.1)))} İŞÇİ GÖNDER`}</button></div>}<div className="world-kingdoms"><div className="section-head"><span>AYNI CHANNEL · {otherKingdoms.length} SANCAK</span><b>İsimler keşifle açılır</b></div>{otherKingdoms.length===0?<p className="empty-world">Bu channel&apos;da henüz başka bir krallık yok.</p>:otherKingdoms.map(kingdom=><article className={kingdom.discovered?"kingdom-contact discovered":"kingdom-contact"} key={kingdom.id}><div><b>{kingdom.name??"Bilinmeyen Sancak"}</b><span>{terrainCatalog[kingdom.terrain as TerrainId]?.label??"BİLİNMEYEN BÖLGE"} · Haritada ({kingdom.position.x}, {kingdom.position.z})</span></div>{kingdom.report?<small>Kale Sv.{kingdom.report.keepLevel} · {kingdom.report.population} nüfus · {kingdom.report.army} asker · {kingdom.report.buildingCount} yapı</small>:kingdom.mission?.status==="pending"?<small>{kingdom.mission.kind==="deep"?"Gözetleme ajanı yolda":"Ajan yolda"} · {left(kingdom.mission.completesAt,now)} · başarı %{kingdom.mission.successChance}</small>:<small>İçerik gizli · keşif başarı ihtimali düşük</small>}
      {/* DERİN GÖZETLEMENİN ÇIKTISI (plan belgesi Fikir 5): kaba durum etiketi,
          SAYI DEĞİL. Rapor bu alanı taşımıyorsa (standart keşif ya da bu alan
          eklenmeden önce alınmış eski rapor) satır hiç çizilmez. */}
      {kingdom.report?.mood?<small className="intel-mood">Halkın hâli · <b>{kingdom.report.mood}</b></small>:null}
      <div className="intel-actions"><button disabled={worldBusy||kingdom.mission?.status==="pending"} onClick={()=>void worldAction("scout",kingdom.id)}>{kingdom.mission?.status==="pending"?"AJAN YOLDA":kingdom.discovered?"YENİDEN KEŞFET":"AJAN GÖNDER"}</button>
      {/* Ayrı ve daha pahalı görev: bedel sunucudan iner, panel sabit
          kodlamaz. Hazine yetmezse sunucu 409 ile reddeder ve mesajı basar. */}
      <button className="deep-scout" disabled={worldBusy||kingdom.mission?.status==="pending"} onClick={()=>void worldAction("deep_scout",kingdom.id)}>DERİN GÖZETLEME · {deepIntel.deepCost} ALTIN</button></div></article>)}</div></div>{game.notices.map((x,i)=><div className="report" key={i}><span>{x.kind}</span><p>{x.text}<small>{new Date(x.at).toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"})}</small></p></div>)}</div>}
 </div></aside>{worldView&&<ChannelWorldMap channelName={game.channel} homeName={game.kingdomName} homeTerrain={game.terrain} homePosition={worldHome} extent={worldExtentValue} homeKeepLevel={lv} kingdoms={otherKingdoms} sharedMine={sharedMine} busy={worldBusy} formatLeft={at=>left(at,now)} onClose={()=>setWorldView(false)} onScout={id=>void worldAction("scout",id)} onMine={action=>void mineAction(action)}/>} {infoPanel&&
  // Geri planı tıklayınca kapanır; role="dialog" aria-query'de etkileşimli
  // sayılmadığı için jsx-a11y bunu statik eleman sanır. Klavye eşdeğeri Esc'tir.
  // eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions
  <div className="info-overlay" role="dialog" aria-modal="true" tabIndex={-1} onClick={()=>setInfoPanel(null)} onKeyDown={e=>{if(e.key==="Escape")setInfoPanel(null)}}>
  {/* Tıklama yalnızca üstteki kapatma tıklamasının karta sızmasını engeller; kartın kendisi bir eylem değildir. */}
  {/* eslint-disable-next-line jsx-a11y/no-static-element-interactions */}
  <div className="info-card" onClick={e=>e.stopPropagation()} onKeyDown={e=>e.stopPropagation()}>
