...
export default function KingdomScene({ night, keepLevel = 1, developed = false, terrain = "plain", constructionName, kingdoms = [], sharedMine }: Props) {
  ...
  useEffect(() => {
    ...
    for(let i=0;i<treeCount;i++){const angle=random()*Math.PI*2,radius=(terrain==="forest"?8:11)+random()*(terrain==="forest"?9:6),x=Math.cos(angle)*radius,z=Math.sin(angle)*radius,s=.7+random()*.65;dummy.position.set(x,.5*s,z);dummy.scale.set(s,s,s);dummy.rotation.y=random()*Math.PI;dummy.updateMatrix();trunks.setMatrixAt(i,dummy.matrix);dummy.position.y=1.7*s;dummy.updateMatrix();crowns.setMatrixAt(i,dummy.matrix)}trunks.castShadow=true;crowns.castShadow=true;detail.add(trunks,crowns);
    const rockCount=terrain==="mountain"?42:16,rocks=new THREE.InstancedMesh(new THREE.DodecahedronGeometry(.45,0),mat(0x6b695e),rockCount);for(let i=0;i<rockCount;i++){const angle=random()*Math.PI*2,radius=6+random()*11;dummy.position.set(Math.cos(angle)*radius,.24,Math.sin(angle)*radius);const s=.5+random()*1.4;dummy.scale.set(s,.65*s,s);dummy.rotation.set(random(),random()*Math.PI,random());dummy.updateMatrix();rocks.setMatrixAt(i,dummy.matrix)}rocks.castShadow=true;detail.add(rocks);
    if(keepLevel===1){
  ...
  },[keepLevel,developed,terrain,constructionName,worldSignature]);
