...
export default function KingdomScene({ night, keepLevel = 1, developed = false, terrain = "plain", constructionName }: Props) {
  ...
  useEffect(() => {
    ...
    const grassCount=terrain==="forest"?260:terrain==="mountain"?90:terrain==="riverbank"?210:180,grass=new THREE.InstancedMesh(new THREE.ConeGeometry(.16,.55,5),mat(terrain==="forest"?0x365c2d:terrain==="mountain"?0x66705a:0x668044),grassCount);
    ...
    if(terrain==="riverbank"){
      const river=add(new THREE.PlaneGeometry(11,130),mat(0x315f70),-28,.01,-8);river.rotation.x=-Math.PI/2;river.rotation.z=.13;
      box(15,.35,3.2,0x765438,-24,.45,-6);for(let i=-6;i<=6;i+=2)box(.22,1.2,.22,0x5d402c,-24+i,.8,-7.5);
      for(let i=0;i<36;i++){const z=-42+i*2.6,side=i%2?1:-1;tree(-28+side*(7+random()*5),z);}
      terrainLabels.push(["BÜYÜK NEHİR",-28,3,-16],["TAHTA KÖPRÜ",-24,3,-6],["VERİMLİ KIYI",-40,3,28]);
    }else if(terrain==="mountain"){
      [[-38,-18,7,13],[-48,12,5,10],[35,22,8,15],[49,-10,6,12],[20,-43,5,10],[-20,45,6,12]].forEach(([x,z,r,h],i)=>{cone(r,h,i%2?0x5d6158:0x6b6a60,x,h/2,z);cone(r*.62,h*.7,0x85837a,x+3,h*.36,z-2);});
      for(let i=0;i<16;i++){const angle=random()*Math.PI*2,radius=30+random()*48;cone(1.2+random()*1.4,2.5+random()*3,0x555a51,Math.cos(angle)*radius,1.8,Math.sin(angle)*radius);}
      terrainLabels.push(["TAŞLIK SIRTLAR",38,11,22],["MADEN DAMARI",-42,5,-16],["DAR GEÇİT",18,4,-42]);
    }else if(terrain==="forest"){
      const clearing=add(new THREE.CircleGeometry(18,40),mat(0x5d713f),0,.01,0);clearing.rotation.x=-Math.PI/2;
      for(let i=0;i<70;i++){const angle=random()*Math.PI*2,radius=19+random()*58;tree(Math.cos(angle)*radius,Math.sin(angle)*radius);}
      const path=add(new THREE.PlaneGeometry(3.4,92),mat(0x7a6846),8,.02,2);path.rotation.x=-Math.PI/2;path.rotation.z=-.24;
      terrainLabels.push(["ESKİ ORMAN",-34,6,18],["AV YOLU",12,3,-26],["ORMAN AÇIKLIĞI",0,3,14]);
    }else{
      [[-42,20,0xb89552],[36,27,0x9b7c45],[-35,-28,0x738442]].forEach(([x,z,color],i)=>{const field=add(new THREE.CircleGeometry(11+i*2,32),mat(color),x,.01,z);field.rotation.x=-Math.PI/2;});
      const road=add(new THREE.PlaneGeometry(4,115),mat(0x8b7750),3,.02,-3);road.rotation.x=-Math.PI/2;road.rotation.z=.72;
      [[42,22],[47,28],[-43,-24]].forEach(([x,z])=>{cone(4.5,7,0x657057,x,3.5,z);cone(3.1,5.2,0x778063,x+4,2.6,z-2);});
      terrainLabels.push(["GENİŞ OVA",-36,3,20],["KERVAN YOLU",18,3,17],["BUĞDAYLIK",36,3,27]);
    }
    const labels:Array<{el:HTMLSpanElement;pos:THREE.Vector3;wide:boolean}>=[];const label=(text:string,x:number,y:number,z:number,wide=false)=>{const el=document.createElement("span");el.className="world-label";el.textContent=text;host.appendChild(el);labels.push({el,pos:new THREE.Vector3(x,y,z),wide});};label(`${keepLevel===1?"AHŞAP KALE":"KALE"} · SV.${keepLevel}`,0,keepLevel===1?5.5:9,0);if(constructionName)label(`⚒ İNŞAAT · ${constructionName}`,constructionName.startsWith("Kale")?0:13,7,constructionName.startsWith("Kale")?0:-3);if(keepLevel>=3)label("TAŞ SUR",5,4,-5);if(keepLevel>=2)label("AMBAR",7,4,11);if(developed)Object.entries(positions).forEach(([name,[x,z]])=>label(name,x,5,z,true));terrainLabels.forEach(([text,x,y,z])=>label(text,x,y,z,true));
