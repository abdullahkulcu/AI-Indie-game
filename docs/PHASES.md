...
| **Fikir 14** | ✅ TAMAM (kesin) | **"General'in verdiği söz" ile Halk'ın gerçek yaşadığı arasındaki fark.** Kralın İLAN ETTİĞİ pay ile ambarın/hazinenin FİİLEN karşıladığı pay arasındaki makas artık bir sinyal: yeni talep türü `vaat`, yiyecek ve maaş kanallarını AYNI kuraldan geçirir (kararın "genel bir desen, tek dar örnek değil" maddesi). **Kapı siyasi:** talep ancak Kral en az TAM payı ilan etmişse (`promise: 100`) açılır — şikâyet "az verdi" değil "verdiğini söylediğini vermedi"dir; Kral istihkakı %70'te tutuyorsa kimseye söz vermemiştir ve o şikâyeti `bread` zaten taşır. Asıl senaryo bu kapının açtığı yerdir: istihkak %150'ye çekilir, General emri GERÇEKTEN uygular, ambar ancak %90 karşılar — `bread` eşiği (85) aşılmadığı için bu madde olmadan halkın hiç sesi çıkmazdı. **KESİN İŞLEM KURALI İHLAL EDİLMİYOR** ve gerekçe `engine/populace-voice.ts` içinde kalıcı bir yorum olarak yazılı (gelecekte biri ilke ihlali sanıp geri almasın): General emri gerçekten çağırdı, ilan edilen oran gerçekten değişti, ortada uygulanmamış eylem yok — kendiliğinden söylemediği şey SONUÇTU. Yani "yanlış iddia" değil "eksik açıklama"; Kral'ın gözünde doğan "General yalan söyledi" hissi, halkın General'in atlamayı seçtiği sonucu masaya koymasıdır. Makas ölçüsü (`promiseGaps`) AYRI ve dışa açık bir saf fonksiyondur: Fikir 18 "halk fark etti mi" sorusunu aynı yerden okur, ikinci bir ölçü açılmaz (kısıt #5). Talep ASLA `urgent` olmaz (kıyasla aynı gerekçe: aciliyeti asıl sıkıntı taşır, makas bir HESAP SORMADIR ve acil bir talebin tavandaki yerini almaz) ve iki kanalda birden makas varsa TEK satır açılır. Metin iki sayıyı yan yana koyar (ikiyüzlülük ancak öyle görünür); Halk-AI yolunda ise `DEMAND_SUBJECT` üzerinden sayısız anlatılır ve kanalı `voice` söyler. **ŞEMAYA DOKUNULMADI:** `populace_demands.kind` serbest metin sütunudur (enum kısıtı yok), yeni tür migration istemez ve `.strict()` save şeması hiç devreye girmez. AI çağrısı Fikir 2'nin mevcut yolundan geçer, yeni sağlayıcı çağrısı eklenmedi. | `c89a8e3` | `engine/populace-voice.ts` (`vaat` türü, `VOICE_THRESHOLDS.vaat`, `promiseGaps`/`promiseDemand`, `VoiceSignals.nominalFood`/`nominalPay`/`servedPay`, ilke-gerilimi yorumu), `app/api/general/route.ts` (`populace.servedPay`, makas doktrini), `components/KingdomGame.tsx` (`kingdomContext` → `servedPay`), `tests/populace-voice.test.ts` |
| **Fikir 6** | ✅ TAMAM (kesin) | **Kritik kararlarda "halkın nabzı" göstergesi.** Kral ağır bir emir verdiğinde General'in itiraz/teyit/ret notunun yanında halkın muhtemel tepkisi de duruyor: kararın SONRASI değil ÖNCESİ görünür oluyor, yani demokratik baskı bir tahmine dönüşüyor. Veto DEĞİL sinyal — emri hiçbir şekilde engellemiyor. **Kararın iki maddesi birebir:** (a) gösterge `elevated` VE `severe` kademelerinde görünür, yalnızca `severe` değil; (b) NİTEL etiket, sayısal tahmin YOK — min-maxing riski bilinçli olarak reddedildi ve bir test bütün kademelerde etiket/gerekçe metinlerinde rakam olmadığını doğruluyor. **YENİ EŞİK UYDURULMADI** (kısıt #5): nabız `engine/faction.ts`'in kendi ölçülerinden okunur — `factionTarget(popularity)` ve `FACTION_THRESHOLDS`. Girdinin `factionTarget` (GİDİŞAT) olması belirleyici: bugünkü baskıya bakmak yanıltıcı olurdu, çünkü rıza yeni çöktüğünde baskı hâlâ 0'dır (üstel birikim zaman alır) ve gösterge tam uyarması gereken anda "halk arkanızda" derdi. Kapı TEK yerde (`pulseNote`) durur; risk kademesi zaten `server/general-risk.ts`'te hesaplandığı için ikinci bir eşik listesi açılmadı. `KingdomSnapshot.factionPressure` OPSİYONELDİR: göndermeyen çağıran göstergeyi kaybetmez, nabız yalnızca rızadan okunur. **TEST DERSİ:** `pulseNote` dışa açıldı çünkü kapı `reviewProposedActions` üzerinden SINANAMIYORDU — `low` kademede o fonksiyon zaten hiç not üretmediği için kapıyı kaldıran mutasyon tek bir testi bile kırmadan geçiyordu (gerekçe kodda yazılı). Yeni UI bloğu, yeni DB sütunu, yeni save alanı ve AI çağrısı YOK. | `74f96a1` | `engine/faction.ts` (`populacePulse`, `PULSES`, `PopulacePulse`), `server/general-risk.ts` (`RiskAssessment.pulse`, `pulseNote` kapısı, `KingdomSnapshot.factionPressure`), `app/api/general/route.ts` (`snapshotOf`), `tests/general-review.test.ts` |
