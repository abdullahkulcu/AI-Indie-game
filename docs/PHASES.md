...
| **Fikir 22** | ✅ TAMAM (kesin) | **Ortak madende nüfuz mücadelesi.** Damarda açık üstünlüğü olan krallık "bölge sahibi" olur ve ötekilerin AKTİF üretiminden %10 pay alır — `engine/mine.ts`'in kuruluşundan beri taşıdığı "kimse başkasının payını yemez" ilkesine getirilen BİLİNÇLİ istisna; kararın ikinci yarısı gereği dosyanın kendi yorumu da yeniden yazıldı (istisna ve SINIRLARI dosya başında açıkça yazılı), çünkü kod bir ilkeyi söylüyorsa ve o ilke artık kısmen geçersizse yorum sessizce eskimiş bırakılmaz. **Ölçüt anlık işçi sayısı DEĞİL:** kapalı çözümlü üstel bir zaman ağırlıklı ortalama (`worker_avg`, τ = 24 oyun saati) — anlık olsaydı krallıklar her hesapta işçi sayısını oynatıp "sahiplik kapma" yarışına girerdi (kararın kendi gerekçesi). **Sahiplik ayrıca pencereye KİLİTLİ:** 6 oyun saatlik, MUTLAK ZAMANA oturan pencerelerin başında bir kez tartılır (`resolveRaids`'in akın penceresi deseni). **KISIT #2 bu maddenin en riskli tarafıydı** ve iki mekanizmayla çözüldü: (a) ortalama kapalı çözümlü üstel olduğu için bölünebilir (`engine/faction.ts`'in gerekçesi), (b) sahiplik eşiği SÜREKSİZ bir karar olduğu için pencereye kilitlendi ve hesap pencere sınırlarından bölünerek yürüyor. İkisi de testli; ikinci test sahipliğin GERÇEKTEN el değiştirdiği bir senaryoda tek adım ile 24 adımın aynı dağıtımı ürettiğini gösteriyor — pencere kilidi kaldırılınca kırıldığı MUTASYONLA doğrulandı (ilk yazdığım pencere testi hiçbir şey yakalamıyordu, gerekçesi `updates/2026-08-23-madende-nufuz-mucadelesi.md`'de). **İstismar frenleri:** nüfuz CEVHER ÜRETMEZ (pay yalnızca yer değiştirir, damarın bakiyesi aynı kalır — testli), madende o an işçisi olmayan krallık pay alamaz ("işçini çek, kirayı yine topla" kapalı), kılpayı öndeki sahiplik almaz, ve ortalama panele İNMEZ (sahipliğin ölçütüdür; sızması "kaç işçiyle geçebilirim" hesabını birebir çözerdi) — yalnızca sahibin adı iner. **Nüfus tavanı (%20) karara uygun olarak DEĞİŞMEDİ.** Save şemasına alan eklenmedi; nüfuzun bütün durumu maden tablolarında yaşıyor. | `b871463` | `engine/mine.ts` (`INFLUENCE_*` sabitleri, `advanceWorkerAvg`, `dominantMiner`, `influenceWindowAt`, `settleMine`'ın pencere döngüsü), `db/schema.ts` + `drizzle/pg/0009_bright_dark_beast.sql` (üç ADD COLUMN + FK, additive), `app/api/mine/route.ts`, `components/KingdomGame.tsx` (`diyar` sekmesi), `app/game.css`, `docs/ARCHITECTURE.md`, `tests/mine-influence.test.ts` |
| **Fikir 23** | ✅ TAMAM (kesin) | **Maden tükenince ortaya çıkan fırsat (define, yıkık kale).** Damar tükendiğinde (`oreRemaining <= 0`) aynı bölgede TOHUMLU bir keşif beliriyor — tür, miktar ve konum channel + tükenme anından `rand01` deseniyle türetilir, admin müdahalesi gerekmez. **Tek kazanan tüm fırsatı alır** (plan kararı): Fikir 22'nin sürekli paylaşımlı doğasından bilinçli olarak farklı bir ton, tutarsızlık değil — maden işbirlikçi+rekabetçi bir yer, tükenme-sonrası olay tek bir an. Kazanan koşullu bir UPDATE ile belirlenir (yarışta ikinci istek boş döner). Ulaşmak için madende işçi bulundurmak şart; kimse almazsa fırsat BEKLEMEDE kalır ve sonradan da alınabilir (affedici tasarım, plan kararı). **Şema yalnızca EKLEYEREK değişti:** yeni `shared_mine_finds` tablosu (`drizzle/pg/0010_natural_gorgon.sql`), mevcut tablolara dokunulmadı ve plandaki "UNIQUE index kaldırılmalı" seçeneğine hiç gerek kalmadı. Save şemasına alan EKLENMEDİ, yani kısıt #3 devreye girmedi. | (bu commit) | `engine/mine.ts` (`mineFindOf` — tohumlu keşif, saf), `db/schema.ts` + `drizzle/pg/0010_natural_gorgon.sql`, `app/api/mine/route.ts`, `components/KingdomGame.tsx` (DİYAR paneli), `tests/mine-find.test.ts` |
